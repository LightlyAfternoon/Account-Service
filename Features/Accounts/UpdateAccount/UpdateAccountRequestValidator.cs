    ...
    public class UpdateAccountRequestValidator : AbstractValidator<UpdateAccountRequestCommand>
    {
        ...
        public UpdateAccountRequestValidator(IUsersService usersService)
        {
            ...
            RuleFor(a => a.InterestRate).Empty()
                .When(a => a.Type.Equals(AccountType.Checking)).WithMessage("Для текущего счёта не может быть процентной ставки");
            ...
            RuleFor(a => usersService.FindById(a.OwnerId)).NotEmpty().WithMessage("Владельца с данным id не существует");
        }
    }
}
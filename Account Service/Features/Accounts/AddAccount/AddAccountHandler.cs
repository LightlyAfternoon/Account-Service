    ...
    public class AddAccountHandler : IRequestHandler<AddAccountRequestCommand, AccountDto?>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Account? account = await _accountsRepository.Save(AccountMappers.MapToEntity(dto), cancellationToken);

                if (account != null)
                {
                    var body = new AccountOpened(eventId: Guid.NewGuid(), occurredAt: DateTime.Now,
